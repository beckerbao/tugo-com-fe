...
$period_start = $today->modify('first day of this month')->modify('-11 months');
$monthly_average_stats = [];
for ($month_index = 0; $month_index < 12; $month_index++) {
    $month = $period_start->modify("+{$month_index} months");
    $month_key = $month->format('Y-m');
    $monthly_average_stats[$month_key] = [
        'label' => $month->format('m/Y'),
        'group_count' => 0,
        'review_count' => 0,
        'average_reviews_per_group' => 0,
    ];
}
foreach ($tour_groups as $tour_group) {
    $start_date = trim((string) ($tour_group['start_date'] ?? ''));
    if ($start_date === '') continue;
    try {
        $tour_date = new DateTimeImmutable($start_date);
    } catch (Exception $exception) {
        continue;
    }
    if ($tour_date < $period_start || $tour_date > $today) continue;
    $month_key = $tour_date->format('Y-m');
    if (!isset($monthly_average_stats[$month_key])) continue;
    $monthly_average_stats[$month_key]['group_count']++;
    $monthly_average_stats[$month_key]['review_count'] += max(0, (int) ($tour_group['post_count'] ?? 0));
}
$average_period_group_count = 0;
$average_period_review_count = 0;
foreach ($monthly_average_stats as &$month_average) {
    $month_average['average_reviews_per_group'] = $month_average['group_count'] > 0
        ? round($month_average['review_count'] / $month_average['group_count'], 2)
        : 0;
    $average_period_group_count += $month_average['group_count'];
    $average_period_review_count += $month_average['review_count'];
}
unset($month_average);
$average_reviews_per_group = $average_period_group_count > 0
    ? round($average_period_review_count / $average_period_group_count, 2)
    : 0;
...
<main class="statistics-shell">
    ...
    <?php else: ?>
        <section class="kpi-grid" aria-label="Các chỉ số chính">
            ...
            <article class="kpi-card"><div class="kpi-label">Review trung bình / đoàn</div><div class="kpi-value"><?php echo number_format($average_reviews_per_group, 2); ?></div><div class="kpi-meta">12 tháng gần nhất</div></article>
        </section>
        ...
        <section class="dashboard-grid">
            ...
            <article class="dashboard-card wide"><div class="card-heading"><div><h2>Review trung bình trên mỗi đoàn</h2><p>Tính theo ngày khởi hành trong 12 tháng gần nhất; tháng không có đoàn hiển thị 0</p></div></div><div class="chart-wrap"><canvas id="monthlyAverageChart"></canvas></div></article>
...
const monthlyAverageStats = <?php echo json_encode(array_values($monthly_average_stats), $json_flags); ?>;
...
if (monthlyAverageStats.length) new Chart(document.getElementById('monthlyAverageChart'), { type: 'bar', data: { labels: monthlyAverageStats.map(item => item.label), datasets: [{ label: 'Review trung bình / đoàn', data: monthlyAverageStats.map(item => item.average_reviews_per_group), backgroundColor: '#6b2f80', borderRadius: 5 }] }, options: { responsive: true, maintainAspectRatio: false, scales: { y: { beginAtZero: true, grid: { color: '#eef1f2' }, ticks: { precision: 0 } } }, plugins: { legend: { display: false }, tooltip: { callbacks: { label: context => `${context.raw} review / đoàn` } } } } });
